...
namespace TaskManagerApp.Infra.Repositories
{
        ...
        where T : Entity, new()
    {
        ...
        public IQueryable<T> QueryOnlyId() => _dbSet.Select(x => new T { Id = x.Id }).AsQueryable().AsSplitQuery();
        ...
        protected void MarkRelationsForDelete<TRelatedEntity>(T entityFromDb, T entityFromRequest, Expression<Func<T, IEnumerable<TRelatedEntity>>> relatedEntitiesSelector)
            where TRelatedEntity : Entity, new()
        {
            if (entityFromDb == null || entityFromRequest == null) return;
            ...
            var entityIdsToDelete = existingRelatedEntities.Select(x => x.Id).Except(relatedEntities.Select(x => x.Id)).ToList();
            var entitiesToDelete = existingRelatedEntities.Where(x => entityIdsToDelete.Contains(x.Id)).ToList();
